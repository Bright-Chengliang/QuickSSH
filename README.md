...
**能力信号：** 这部分体现的不是简单表单，而是对配置层级、实体关系和数据迁移的完整建模。
...
**能力信号：** 这里展示的是 Android 生命周期、异步连接、终端协议处理和用户输入模型之间的协同，而不是一个单纯的 WebView 外壳。
...
**能力信号：** 这部分体现了对后台任务、状态机、失败恢复和第三方库边界的工程处理。
...
**能力信号：** 这体现了网络拓扑、后台服务和移动端交互之间的结合，也明确了“通过隧道访问”与“直接暴露服务”的边界。
...
**能力信号：** 这部分体现了密钥生命周期、认证材料保护、主机身份验证和可恢复备份之间的组合设计。
